...
API_TOKEN = '6793723578:AAEqEC3-NkoXcFusRYkZCGpa6t_Wh8uCFAc'
...
def webAppKeyboard(user_data):
    ...
    webAppTest = types.WebAppInfo(f"https://689c30cb3259.ngrok.app/webapp.html?username={username}&first_name={first_name}&last_name={last_name}")
...
def send_welcome(message):
    ...
    if user_from_db:
        bot.send_message(message.chat.id, 'Hello! Click the button to open the web app.', reply_markup=webAppKeyboard(user_from_db))
    else:
        ...
        try:
            ...
            if event == "ask_question":
                ...
                try:
                    ...
                    if query_id:
                        bot.answer_web_app_query(query_id, json.dumps({"action": "question_received", "status": "success"}))
                ...
                try:
                    ...
                    if query_id:
                        bot.answer_web_app_query(query_id, json.dumps({"action": "profile_data", "data": user_questions}))
                except Exception as e:
